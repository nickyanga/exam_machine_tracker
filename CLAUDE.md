...
> State persistence goes through a `store` adapter that prefers `window.storage` (available natively in the Claude Code desktop app) and falls back to the browser's `localStorage`. State persists between refreshes in both environments.
...
- **State** — machine statuses (`"rem"`, `"prog"`, `"done"`) are stored in the `states` object keyed by asset tag, persisted via the `store` adapter (`window.storage` with `localStorage` fallback) under key `exam_states_v2`. The seed flag uses `exam_seeded_v2`.
...
1. On load, `store.get(STORE_KEY)` restores saved states.
2. If never seeded before, `initiallyDone` field in `MACHINES` seeds initial state and sets the seed flag.
3. Each card click cycles: `rem → prog → done → rem`, saves immediately via `store.set`.
